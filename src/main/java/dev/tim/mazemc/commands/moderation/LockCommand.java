...
public class LockCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        if(event.getName().equalsIgnoreCase("lock")){

            if(event.getMember().hasPermission(Permission.MESSAGE_MANAGE)) {

                Guild guild = event.getGuild();

                event.getTextChannel().getManager().putRolePermissionOverride(guild.getPublicRole().getIdLong(), null, EnumSet.of(Permission.MESSAGE_SEND)).queue();

                EmbedBuilder embed = new EmbedBuilder();
                embed.setTitle("`\uD83D\uDD12` | Kanaal gelocked!");
                ...
                logEmbed.setTitle("Kanaal gelocked!");
