...
public class SuggestAcceptCommand extends ListenerAdapter {

    @Override
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        if(event.getName().equalsIgnoreCase("suggestaccept")){
            ...
            if(event.getMember().hasPermission(Permission.MESSAGE_MANAGE)) {
                ...
                event.getGuild().getTextChannelById(Utils.SUGGEST_ACCEPTED_CHANNEL_ID).sendMessageEmbeds(suggestionChannel.retrieveMessageById(id).complete().getEmbeds()).queue();

                EmbedBuilder embedAccepted = new EmbedBuilder();
                embedAccepted.setTitle("✅ | Suggestie Geaccepteerd");
                embedAccepted.addField("Reden:", reason, false);
                embedAccepted.addField("Door:", event.getMember().getAsMention(), false);
                embedAccepted.setColor(Color.GREEN);
                embedAccepted.setFooter("MazeMC", event.getGuild().getIconUrl());
                event.getGuild().getTextChannelById(Utils.SUGGEST_ACCEPTED_CHANNEL_ID).sendMessageEmbeds(embedAccepted.build()).queue();

                event.reply("Suggestie succesvol geaccepteerd!").queue();
